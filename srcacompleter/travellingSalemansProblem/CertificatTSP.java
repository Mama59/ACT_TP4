...
// A completer
	...
	public void saisie() {
		...
		while (saisie.length() != this.tsp.nbVilles) {
			System.out.println("Vous devez entrer exactement " + this.tsp.nbVilles + " caract�res");
			saisie = sc.nextLine();
		}
	...
	public void suivant() {
		...
		boolean exit = false;
		// on parcourt la liste en commen�ant par le dernier caract
		for (int i = this.liste.size() - 1; i >= 0; i--) {
			if (exit) {
				return;
			}
			// si pas le caract max, on ncr�mente le caract
			if (this.liste.get(i) != this.valeursPossibles.get(this.valeursPossibles.size() - 1)) {
				this.liste.set(i, this.valeursPossibles.get(this.valeursPossibles.indexOf(this.liste.get(i)) + 1));
				exit = true;
			}
			// sinon, c'ets le dernier caract possible, on ins�re le premier et passe � la ville prec�dente
			else {
